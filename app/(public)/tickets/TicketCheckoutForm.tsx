...
export default function TicketCheckoutForm({ performances }: { performances: TicketPerf[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/tickets/checkout', {
        ...
        body: JSON.stringify({
          items: selectedItems.map(p => ({ ticket_performance_id: p.id, quantity: quantities[p.id] })),
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button
        type="submit"
        disabled={loading || !hasSelection || !name.trim() || !email.trim()}
        style={{
          width: '100%',
          padding: '16px',
          ...
          cursor: loading ? 'wait' : !hasSelection || !name.trim() || !email.trim() ? 'not-allowed' : 'pointer',
          opacity: !hasSelection || !name.trim() || !email.trim() ? 0.5 : 1,
